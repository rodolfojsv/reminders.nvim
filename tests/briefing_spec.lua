...
describe("reminders.briefing", function()
	...
	describe("once_daily tracking", function()
		it("shown_today returns false when no state file exists", function()
			assert.is_false(briefing.shown_today())
		end)

		it("shown_today returns true after mark_shown_today", function()
			briefing.mark_shown_today()
			assert.is_true(briefing.shown_today())
		end)

		it("shown_today returns false when state file has a different date", function()
			local path = tmp_dir .. "/briefing_last_shown"
		...
		it("shown_today returns false when directory_path is nil", function()
			config.setup({ directory_path = nil })
			briefing = require("reminders.briefing")
			assert.is_false(briefing.shown_today())
		end)
	end)
